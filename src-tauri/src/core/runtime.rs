...
pub async fn start_input_loop(
    ...
    while let Some(event) = event_rx.recv().await {
        // Check if focus switched — if so, reset modifier state to prevent stale keys
        ...
        if current_focus != last_focus {
            if matches!(current_focus, FocusState::Local) {
                // Sync modifier state with actual keyboard when returning to local focus
                ...
                // Switching away from local — reset modifiers
                ctrl_held = false;
                cmd_held = false;
            }
