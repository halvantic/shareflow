...
### 4.1 · Deduplicate peer session message-handling code `[x]`
- **Severity:** High  
...
### 4.2 · Implement application-layer pairing handshake `[x]`
- **Severity:** High  
...
### 5.1 · Encrypt AMT/IPMI credentials at rest `[x]`
- **Severity:** Critical  
...
### 5.2 · File transfer: add SHA-256 integrity verification `[x]`
- **Severity:** High  
...
### 6.1 · Add per-peer input event rate limiting `[x]`
- **Severity:** Critical  
...
### 6.2 · Fix message queue lo-priority starvation `[x]`
- **Severity:** High  
...
### 6.3 · macOS: fix KEYBOARD_PRIMED startup race condition `[x]`
- **Severity:** Medium  
...
### 6.4 · macOS: fix modifier key tracking (Caps Lock mapping + synthetic release race) `[x]`
- **Severity:** Medium  
...
### 6.5 · macOS: fix scroll delta precision loss `[x]`
- **Severity:** Medium  
...
### 6.6 · macOS: log unmapped keycode instead of sending scancode 0 `[x]`
- **Severity:** Medium  
...
### 6.7 · Windows: fix remote bounds race condition `[x]`
- **Severity:** Medium  
...
### 6.8 · Windows: fix SUPPRESS / ShowCursor ordering `[x]`
- **Severity:** Medium  
...
### 6.9 · Fix atomic ordering for primary_km and is_remote flags `[x]`
- **Severity:** Medium  
...
### 6.10 · Fix focus state TOCTOU for non-primary K+M device `[x]`
- **Severity:** High  
...
### 7.1 · Config deserialization: log and alert on silent reset `[x]`
- **Severity:** Medium  
...
### 7.2 · Windows private key file permissions `[x]`
- **Severity:** Medium  
...
### 7.3 · Add startup initialisation barrier `[x]`
- **Severity:** Medium  
...
### 7.4 · Unsafe Objective-C FFI: add null guards `[x]`
- **Severity:** Low  
...
### 7.5 · Commit Cargo.lock and add cargo audit `[x]`
- **Severity:** Medium  
...
### 7.6 · Linux data directory path: follow XDG spec `[x]`
- **Severity:** Low  
...
### 7.7 · Discovery replay: add per-announcement nonce `[x]`
- **Severity:** Low  
...
### 7.8 · Restrict crash log file permissions `[x]`
- **Severity:** Low  
...
### 7.9 · macOS: re-check Accessibility permission at runtime `[x]`
- **Severity:** Low  
...
### 8.1 · AMT: add status tracking, HTTPS, and audit logging `[x]`
- **Severity:** Design  
...
### 8.2 · Implement per-peer input event rate limiting `[x]`
...
### 8.5 · Make config struct self-enforcing (invalid state prevention) `[x]`
- **Severity:** Design  
...
### 8.6 · Implement hotkey configuration and wire up detector `[x]`
- **Severity:** Design  
